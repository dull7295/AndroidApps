...
public class DemoNoSQLTableNotes extends DemoNoSQLTableBase {
    private static final String LOG_TAG = DemoNoSQLTableNotes.class.getSimpleName();
    ...
    public class DemoGetWithPartitionKeyAndSortKey extends DemoNoSQLOperationBase {
        ...
        DemoGetWithPartitionKeyAndSortKey(final Context context) {
            super(context.getString(R.string.nosql_operation_get_by_partition_and_sort_text),
                String.format(context.getString(R.string.nosql_operation_example_get_by_partition_and_sort_text),
                    ...
                    "noteId", "demo-userId-500000"));
        }

        @Override
        public boolean executeOperation() {
            // Retrieve an item by passing the partition key using the object mapper.
            result = mapper.load(NotesDO.class, AWSMobileClient.defaultMobileClient().getIdentityManager().getCachedUserID(), "demo-noteId-500000");
        ...
        public List<DemoNoSQLResult> getNextResultGroup() {
            ...
            results.add(new DemoNoSQLNotesResult(result));
    ...
    public class DemoQueryWithPartitionKeyAndSortKeyCondition extends DemoNoSQLOperationBase {
        ...
        private PaginatedQueryList<NotesDO> results;
        private Iterator<NotesDO> resultsIterator;
        ...
        public boolean executeOperation() {
            ...
            final Condition rangeKeyCondition = new Condition()
                ...
                .withAttributeValueList(new AttributeValue().withS("demo-noteId-500000"));
            ...
            results = mapper.query(NotesDO.class, queryExpression);
            if (results != null) {
    ...
    public class DemoQueryWithPartitionKeyAndFilter extends DemoNoSQLOperationBase {
        ...
        public boolean executeOperation() {
            ...
            filterExpressionAttributeNames.put("#content", "content");

            final Map<String, AttributeValue> filterExpressionAttributeValues = new HashMap<>();
            filterExpressionAttributeValues.put(":Mincontent",
                new AttributeValue().withS("demo-content-500000"));
    ...
    public class DemoDateSortedQueryWithPartitionKeyAndSortKeyCondition extends DemoNoSQLOperationBase {
        ...
        public boolean executeOperation() {
            ...
            DynamoDBQueryExpression<NotesDO> queryExpression = new DynamoDBQueryExpression<NotesDO>()
                .withHashKeyValues(itemToFind)
    ...
    public class DemoDateSortedQueryWithPartitionKeyOnly extends DemoNoSQLOperationBase {
    ...
    public class DemoDateSortedQueryWithPartitionKeyAndFilterCondition extends DemoNoSQLOperationBase {
        ...
        public boolean executeOperation() {
            ...
            // colliding with DynamoDB reserved words.
            final Map <String, String> filterExpressionAttributeNames = new HashMap<>();
            filterExpressionAttributeNames.put("#noteId", "noteId");

            final Map<String, AttributeValue> filterExpressionAttributeValues = new HashMap<>();
            filterExpressionAttributeValues.put(":MinnoteId",
                new AttributeValue().withS("demo-noteId-500000"));

            // Perform get using Partition key and sort key condition
            DynamoDBQueryExpression<NotesDO> queryExpression = new DynamoDBQueryExpression<NotesDO>()
                .withHashKeyValues(itemToFind)
                .withFilterExpression("#noteId > :MinnoteId")
                .withExpressionAttributeNames(filterExpressionAttributeNames)
                .withExpressionAttributeValues(filterExpressionAttributeValues)
                .withConsistentRead(false);
            results = mapper.query(NotesDO.class, queryExpression);
    ...
    public class DemoDateSortedQueryWithPartitionKeySortKeyAndFilterCondition extends DemoNoSQLOperationBase {
    ...
    public class DemoScanWithFilter extends DemoNoSQLOperationBase {
        ...
        private PaginatedScanList<NotesDO> results;
        private Iterator<NotesDO> resultsIterator;
        ...
        public boolean executeOperation() {
            ...
            results = mapper.scan(NotesDO.class, scanExpression);
            if (results != null) {
                resultsIterator = results.iterator();
                if (resultsIterator.hasNext()) {
                    return true;
     ...
     */
    private static List<DemoNoSQLResult> getNextResultsGroupFromIterator(final Iterator<NotesDO> resultsIterator) {
        if (!resultsIterator.hasNext()) {
            return null;
        }
        ...
        do {
            ...
            resultGroup.add(new DemoNoSQLNotesResult(item));
            itemsRetrieved++;
    ...
    public void insertSampleData() throws AmazonClientException {
        ...
        firstItem.setCreationDate(DemoSampleDataGenerator.getRandomSampleNumber());
        firstItem.setTitle(
        ...
        final NotesDO[] items = new NotesDO[SAMPLE_DATA_ENTRIES_PER_INSERT-1];
        for (int count = 0; count < SAMPLE_DATA_ENTRIES_PER_INSERT-1; count++) {
            final NotesDO item = new NotesDO();
            item.setUserId(AWSMobileClient.defaultMobileClient().getIdentityManager().getCachedUserID());
            item.setNoteId(DemoSampleDataGenerator.getRandomSampleString("noteId"));
